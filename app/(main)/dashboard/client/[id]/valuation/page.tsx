...
import { checkIpoValuationAction, createIpoValuationAction } from "@/lib/data/valuationAction";
...
export default function WithoutProspectusPage() {
  ...
  const [categoryName, setCategoryName] = useState<
    "Company Narrative & Growth Strategy" | "Risk Management & Due Diligence"
  ...
  const { riskDocuments, narrativeDocuments, allAuditedFinancials } =
    useMemo(() => {
  ...
  useEffect(() => {
    if (clientData === null && isLoading) return;

    const checkValuation = async () => {
      try {
        setLoadingValuationCheck(true);
        const valuationCheckRes = await checkIpoValuationAction({
          clientId: clientData!.id,
        });
        console.log("Valuation check response:", valuationCheckRes);
        if (valuationCheckRes.success) {
          setValuationExists(true);
          setValuationData(valuationCheckRes.data);
          setReportLink(valuationCheckRes.publicUrl);
        } else if (valuationCheckRes.msg === "Report Generating") {
          setValuationProcessing(true);
          setUpdateAt(
            valuationCheckRes.data?.updatedAt
              ? new Date(valuationCheckRes.data.updatedAt)
              : null
          );
        } else {
  ...
  const handleValuationReportGeneration = async () => {
    try {
      ...
      const res = await createIpoValuationAction(payload);

      //Now Run the Endpoint /get-structured-data?generation_id=${data.generation_id}&client_account_id=${data.clientAccountId}

      setValuationData(res.value);
      setValuationProcessing(true);
      setUpdateAt(
        res.value?.updatedAt ? new Date(res.value.updatedAt) : null
      );

      if (res.success) {
        // Prepare URL
        const url = `http://localhost:8000/api/v1/get-structured-data?generation_id=${res.value.generation_id}&client_account_id=${res.value.clientAccountId}`;
  ...
  if (valuationExists) {
    ...
    const formattedUpdateAt = updateAt
      ? updateAt.toLocaleDateString(undefined, {
          year: "numeric",
          month: "short",
          day: "numeric",
        })
    ...
    const BaseValuation =
      out.base_case_valuation * 0.5 + out.base_intrinsic_value * 0.5;
    ...
    return (
      <div className="max-w-4xl mx-auto px-6 py-10 bg-white shadow-lg rounded-xl">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-8">
          <div className="flex flex-col items-center md:items-start">
            ...
            <p className="text-gray-600 mt-1">
              Read or download your valuation report below.
            </p>
          </div>
        ...
        <div className="mt-4 text-gray-700 space-y-1">
          <p>
            <span className="font-semibold">For:</span> {out.company_name}
          </p>
          <p>
            <span className="font-semibold">Date:</span> {formattedUpdateAt}
          </p>
        </div>
        {/* </div> */}
          ...
          <ul className="list-disc pl-6 text-gray-800 space-y-2">
            ...
            <li>
              ...
              <span className="font-bold">
                {" "}
                ${round(Final_Calculation_R * 0.85, 2)} to $
                {round(Final_Calculation_R * 1.15, 2)} per share
              </span>
            </li>
            <li>
              <span className="font-semibold">Key Takeaway:</span>
              This valuation is driven by strong intrinsic value and excellent
              financial health, adjusted downward for documented market risks
              including major customer concentration.
            </li>
          </ul>
        </div>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className="border px-4 py-2">
                  ${round(out.base_intrinsic_value, 4)}
                </td>
                <td className="border px-4 py-2">50%</td>
                <td className="border px-4 py-2">
                  ${round(out.base_intrinsic_value * 0.5, 4)}
                </td>
              </tr>
              <tr>
                <td className="border px-4 py-2">CCA Market Value</td>
                <td className="border px-4 py-2">
                  ${round(out.base_case_valuation, 4)}
                </td>
                <td className="border px-4 py-2">50%</td>
                <td className="border px-4 py-2">
                  ${round(out.base_case_valuation * 0.5, 4)}
                </td>
              </tr>
              ...
              <tr className="bg-blue-50 font-semibold">
                ...
                <td className="border px-4 py-2"></td>
                <td className="border px-4 py-2">
                  ${Qualitative_Adjustment_R}%
                </td>
              </tr>
              <tr>
                <td className="border px-4 py-2">Financial Health Score</td>
                <td className="border px-4 py-2">
                  {out.overall_financial_health_score}/100
                </td>
              ...
              <tr>
                ...
                <td className="border px-4 py-2">
                  {out.overall_narative_sentiment_score}/100
                </td>
              ...
              <tr>
                ...
                <td className="border px-4 py-2">
                  {out.overall_risk_score}/100
                </td>
                <td className="border px-4 py-2">30%</td>
                <td className="border px-4 py-2 italic">
                  Contributes discount
                </td>
              </tr>
              ...
              <tr>
                <td className="border px-4 py-2">
                  Base Valuation × (1 + Adjustment)
                </td>
                <td className="border px-4 py-2">
                  ${BaseValuation_R} × {1 + Qualitative_Adjustment}
                </td>
                <td className="border px-4 py-2"></td>
                <td className="border px-4 py-2">${Final_Calculation_R}</td>
              </tr>
          ...
          <ul className="list-disc pl-6 space-y-4 text-gray-800">
            <li>
              ...
              <ul className="pl-6 list-disc mt-2">
                <li>
                  Intrinsic value depends on a{" "}
                  <strong>2.5% terminal growth rate</strong>
            ...
            <li>
              <span className="font-semibold text-gray-900">
                From Comparable Company Analysis (CCA):
              </span>
              <ul className="pl-6 list-disc mt-2">
                <li>
                  Peer tech companies trade at a{" "}
                  <strong>median EV/Revenue of 2.68x</strong>.
                </li>
              </ul>
            </li>
            ...
            <li>
              <span className="font-semibold text-gray-900">
                From the Financial Health Scorecard:
              </span>
              <ul className="pl-6 list-disc mt-2">
                <li>
                  Financial health is <strong>excellent (94.3/100)</strong>,
                  driven by 2-year Revenue CAGR of <strong>22.47%</strong>.
                </li>
              </ul>
            </li>
            ...
            <li>
              <span className="font-semibold text-gray-900">
                From the Narrative Strength Report:
              </span>
              <ul className="pl-6 list-disc mt-2">
                <li>Narrative is strong but lacks specificity.</li>
                <li>
                  <strong>Quantify market size</strong> and{" "}
                  <strong>name competitors</strong> to strengthen pitch
                  documents.
                </li>
              </ul>
            </li>
            ...
            <li>
              <span className="font-semibold text-gray-900">
                From the Risk Heatmap:
              </span>
              <ul className="pl-6 list-disc mt-2">
                <li>
                  Primary risk is <strong>Market Risk (Score: 88/100)</strong>{" "}
                  due to <strong>75% revenue concentration</strong> from a
                  single client.
                </li>
              </ul>
            </li>
        ...
        <div className="mt-10">
          ...
          <p className="text-gray-700 leading-relaxed">
            This valuation is an indicative estimate for strategic planning and
            is not a formal fairness opinion. The final IPO price will depend on
            market conditions, investor demand, and Lead Manager negotiations.
            The valuation was created using a hybrid intrinsic (DCF) + market
            (CCA) model with qualitative adjustments derived from NLP analysis
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 max-w-3xl">
                    ...
                    {narrativeDocuments.map((doc) => (
                      ...
                      >
                        ...
                        <span className="text-gray-500">
                          {splitCamelCase(doc.documentType)}
                        </span>
                      </li>
                    ))}
                  </ul>
      ...
      <DocumentUploadDialog
        ...
        isIPO={true}
        type={
          categoryName === "Company Narrative & Growth Strategy"
            ? DocumentType.InvestorPresentationPitchDeck
            : DocumentType.InternalRiskRegister
        }
      />
