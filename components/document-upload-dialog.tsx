...
export function DocumentUploadDialog({
  ...
  const [documentType, setDocumentType] = useState<DocumentType>(
    documentCategories[
      categoryName as keyof typeof documentCategories
    ][0] as DocumentType
  );
      ...
      const uploadPromises = selectedFile.map((file) =>
        dispatch(
          createDocumentClient({
            Document: { ...file, documentType, periodYear: Number(year) },
            sessionToken,
            isIPO,
          })
        )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[520px] max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a document for {categoryName}. Please select the document
            type and year.
          </DialogDescription>
        </DialogHeader>
