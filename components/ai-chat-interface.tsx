...
export function AIChatInterface({ context, generationId, documentId }: AIChatInterfaceProps) {
  ...
  const {
    ...
    api: `/api/chat?generationId=${encodeURIComponent(generationId)}&documentId=${encodeURIComponent(documentId)}`,
    maxSteps: 2,
  });
  ...
  useEffect(() => {
    append({
      role: "system",
      content: `You're currently reviewing a compliance finding related to:
      - **Title:** ${context.title}
                      ...
                      {part.type === "tool-invocation" && (
                        part.toolInvocation.state !== "result" ? (
                          <span className="text-gray-500 italic">
                            ...
                            ✅ Successfully called tool: {part.toolInvocation.toolName}
                            </span>
                        )
                      )}
                    </span>
